...
function unPinNote() {
  const targetIndex = pinnedArr.findIndex(
    (note) => targetUpdatingNote.id == note.id
  );
...
export function showNotePreview(showId) {
  const targetedNote = arr.find(
    (note) => parseInt(showId) === parseInt(note.id)
  );
...
export function showPinnedNotePreview(showId) {
  const targetedNote = pinnedArr.find(
    (note) => parseInt(showId) === parseInt(note.id)
  );
